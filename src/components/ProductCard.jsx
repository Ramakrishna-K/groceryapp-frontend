...
const ProductCard = ({ product }) => {
  const { addToCart, removeFromCart, cartItems, navigate, axios } = useAppContext();
  ...
  const updateCartBackend = async (updatedCart) => {
    ...
    try {
      ...
      console.log("Cart update error:", err.response?.data || err.message);
    }
  };

  // Wrapper functions to sync cart after adding/removing
