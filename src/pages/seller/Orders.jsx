...
const Orders = () => {
  ...
  const [orders, setOrders] = useState([]);
  const { axios, cartItems } = useContext(AppContext); // ✅ included cartItems

  // ---------------- FETCH ORDERS ----------------
  const fetchOrders = async () => {
    try {
      const { data } = await axios.get(
        `https://groceryapp-backend-552v.onrender.com/api/order/seller`
      );
      if (data.success) {
        setOrders(data.orders);
        // ✅ sync cart whenever orders are fetched
        syncCartToBackend();
      } else toast.error(data.message);
    } catch (error) {
      toast.error(error.message);
