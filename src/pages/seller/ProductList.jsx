...
//     try {
//       const { data } = await axios.post(`https://groceryapp-backend-552v.onrender.com/api/product/stock`, { id, inStock });
//       if (data.success) {
//         fetchProducts();
...
const ProductList = () => {
  const { products, fetchProducts, axios, cartItems } = useAppContext(); // ✅ included cartItems

  // ---------------- TOGGLE STOCK ----------------
  const toggleStock = async (id, inStock) => {
    try {
      const { data } = await axios.post(
        `https://groceryapp-backend-552v.onrender.com/api/product/stock`,
        { id, inStock }
      );
      ...
      if (data.success) {
        fetchProducts();
        toast.success(data.message);
        syncCartToBackend(); // ✅ sync cart whenever stock changes
      } else {
        toast.error(data.message);
      }
    } catch (error) {
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-sm font-semibold">₹{product.offerPrice}</p>
              </div>
            </div>
