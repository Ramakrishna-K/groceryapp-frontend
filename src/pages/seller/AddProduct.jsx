...
//       const { data } = await axios.post(`https://groceryapp-backend-552v.onrender.com/api/product/add-product`, formData);
//       if (data.success) {
...
const AddProduct = () => {
  const { axios, cartItems } = useContext(AppContext); // ✅ added cartItems
  const [files, setFiles] = useState([]);
  const [name, setName] = useState("");
  const [description, setDescription] = useState("");
  const [category, setCategory] = useState("");
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const { data } = await axios.post(
        `https://groceryapp-backend-552v.onrender.com/api/product/add-product`,
        formData
      );
      ...
      if (data.success) {
        ...
        // ✅ Sync cart after product is added
